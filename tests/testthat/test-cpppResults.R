...
test_that("newCpppResult creates a cpppResult with expected fields", {
  x <- cppp::newCpppResult(CPPP = 0.4, repPPP = runif(5), obsPPP = 0.3)
...
test_that("newCpppResult enforces bounds when finite", {
  expect_error(cppp::newCpppResult(CPPP = 1.2), "in \\[0,1\\]")
  expect_error(cppp::newCpppResult(repPPP = c(-0.1, 0.2)), "in \\[0,1\\]")
  expect_error(cppp::newCpppResult(obsPPP = c(0.5, 1.1)), "in \\[0,1\\]")
})
